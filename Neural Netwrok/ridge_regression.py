...
class RidgeRegression(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, l2_reg=1, step_size=.005,  max_num_epochs = 5000):
        ...
        self.squared_distance = nodes.SquaredL2DistanceNode(a=self.prediction, b=self.y, node_name="Squared L2 Distance")
        self.l2_penalty = nodes.L2NormPenaltyNode(l2_reg=self.l2_reg, w=self.w, node_name="L2 Norm Penalty")
        self.objective = nodes.SumNode(a=self.squared_distance, b=self.l2_penalty, node_name="Ridge Loss")
...
def main():
    ...
    #report the average square error
    y_train_predict_1 = estimator.predict(X_train)
    mse_1 = ((y_train_predict_1 - y_train)**2).mean(axis=None)
    print("The average square error when l2reg = {} is {}.".format(l2reg, mse_1))
    ...
    #report the average square error
    y_train_predict_2 = estimator.predict(X_train)
    mse_2 = ((y_train_predict_2 - y_train)**2).mean(axis=None)
    print("The average square error when l2reg = {} is {}.".format(l2reg, mse_2))
