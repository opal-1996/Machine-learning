...
class MulticlassClassifier(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, num_hidden_units=10, step_size=.005, init_param_scale=0.01, max_num_epochs = 1000, num_class=3):
        ...
        self.affine1 = nodes.AffineNode(self.W1, self.x, self.b1, "affine transformation 1")
        self.tanh = nodes.TanhNode(self.affine1, "tanh transformation")

        self.W2 = nodes.ValueNode(node_name="W2")#hold a parameter matrix
        self.b2 = nodes.ValueNode(node_name="b2")#hold a bias vector
        self.prediction = nodes.AffineNode(self.W2, self.tanh, self.b2, "affine transformation 2")
        self.softmax = nodes.SoftmaxNode(self.prediction , "softmax transformation")

        self.y = nodes.ValueNode(node_name="y") # to hold a scalar response
        self.objective = nodes.NLLNode(self.softmax, self.y, "negative log likelihood")
