...
class MLPRegression(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, num_hidden_units=10, step_size=.005, init_param_scale=0.01, max_num_epochs = 5000):
        ...
        self.x = nodes.ValueNode(node_name="x")#hold a vector input
        self.W1 = nodes.ValueNode(node_name="W1")#hold a parameter matrix
        self.b1 = nodes.ValueNode(node_name="b1")#hold a bias vector

        self.w2 = nodes.ValueNode(node_name="w2")#hold a parameter vector
        self.b2 = nodes.ValueNode(node_name="b2")#hold a bias scalar
        self.y = nodes.ValueNode(node_name="y")#hold a scalar response

        self.affine = nodes.AffineNode(self.W1, self.x, self.b1, "affine transformation")
        self.tanh = nodes.TanhNode(self.affine, "tanh transformation")
        self.prediction = nodes.VectorScalarAffineNode(self.tanh, self.w2, self.b2, "prediction")
        self.objective = nodes.SquaredL2DistanceNode(self.prediction, self.y, "squared loss")
