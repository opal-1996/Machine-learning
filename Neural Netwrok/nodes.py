...
class VectorScalarAffineNode(object):
    ...
    def __init__(self, x, w, b, node_name):
        ...
        b: node for which b.out is a numpy scalar (i.e. 0 dim array)
        node_name: node's name (a string)
        """
        self.node_name = node_name
        self.out = None
...
class L2NormPenaltyNode(object):
    ...
    def backward(self):
        d_x = self.d_out * 2 * self.l2_reg * self.w.out
        self.w.d_out += d_x
        return self.d_out
...
class AffineNode(object):
    ...
    def backward(self):
        ...
        self.x.d_out += d_x
        d_W = self.d_out.reshape(-1,1) @ self.x.out.reshape(-1,1).T
        self.W.d_out += d_W
        d_b = self.d_out * 1
        self.b.d_out += d_b
        return self.d_out
...
class SoftmaxNode(object):
    ...
    def backward(self):
        softmax= np.reshape(self.out, (1, -1))
        d_z = (softmax * np.identity(softmax.size) - softmax.transpose() @ softmax) @ self.d_out
        self.z.d_out += d_z
        return self.d_out
...
class NLLNode(object):
    ...
    def backward(self):
        tmp = np.zeros(self.y_hat.out.shape)
        np.put(tmp,[self.y_true.out], [-1/self.y_hat.out[self.y_true.out]])
        d_hat = self.d_out * tmp
